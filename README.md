...
Server-side routing gives us info when we specifically request it via a url while client-side downloads all the info on the initial page load and renders the relevant info from memory when necessary. Server-side needs to refresh the page after each request while client-side just has to display something else.
